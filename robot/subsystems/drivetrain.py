...
from ctre import NeutralMode, FeedbackDevice, InvertType, WPI_TalonFX, StatorCurrentLimitConfiguration
...
class Drivetrain:
    ...
    def get_left_encoder_vel(self):
        return self.talon_to_meters(self._left_motor.getSelectedSensorVelocity() / DRIVE_ENC_CPR * 10)

    def get_right_encoder_vel(self):
        return self.talon_to_meters(self._right_motor.getSelectedSensorVelocity() / DRIVE_ENC_CPR * 10)

    def get_left_encoder_pos(self):
        return self.talon_to_meters(self._left_motor.getSelectedSensorPosition() / DRIVE_ENC_CPR)

    def tank_drive_volts(self, left: float, right: float) -> None:
        self._left_motor.setVoltage(left)
        self._right_motor.setVoltage(right)
        print("volts", left, right)
